...
(ert-deftest scholia-core-deletes-a-reply-chosen-like-a-reply-target ()
  "Delete asks which annotation of the thread goes, as reply does.
A reply goes with the replies under it and stays gone after a reload;
with no replies the annotation itself goes without asking."
  (scholia-test-with-session-directory
    (scholia-test-with-temp-file-buffer _buffer scholia-core-test--source
      (setq-local scholia-session "delete-thread")
      (setq-local scholia-project-root-function (lambda () nil))
      (scholia-mode 1)
      (goto-char 1)
      (scholia-annotate "the note")
             ...
             (texts (lambda ()
                      (let ((chain (car (scholia-buffer-chains))))
                        (mapcar (lambda (entry)
                                ...
                                (alist-get (overlay-get (car chain) 'scholia--chain-id)
                                           scholia--replies))))))
        (scholia-reply-to "first" root)
        (let ((first (scholia-db-annotation-id
                      (cdr (car (alist-get (overlay-get (car chain) 'scholia--chain-id)
                                           scholia--replies))))))
          (scholia-reply-to "under first" first)
          (scholia-reply-to "second" root)
          (let ((asked nil))
            (cl-letf (((symbol-function 'completing-read)
                       (lambda (prompt candidates &rest _)
                         (setq asked prompt)
                         (car (nth 1 candidates)))))
              (call-interactively #'scholia-delete-annotation))
            (should (equal asked "Delete: "))))
        (should (equal (funcall texts) '((1 . "second"))))
        (scholia-mode -1)
        (scholia-mode 1)
        (should (equal (funcall texts) '((1 . "second"))))
        (let ((asked nil))
          (cl-letf (((symbol-function 'completing-read)
                     (lambda (&rest _) (setq asked t) (error "Asked"))))
            (goto-char 1)
            (let ((chain (car (scholia-buffer-chains))))
...
(ert-deftest scholia-core-edits-a-reply-chosen-like-a-reply-target ()
  "Edit asks which annotation of the thread changes, as reply and delete do.
A reply keeps its place and its new text after a reload."
  (scholia-test-with-session-directory
    (scholia-test-with-temp-file-buffer _buffer scholia-core-test--source
      (setq-local scholia-session "edit-thread")
      (setq-local scholia-project-root-function (lambda () nil))
      (setq-local scholia-annotation-editor 'minibuffer)
      (scholia-mode 1)
      (goto-char 1)
      (scholia-annotate "the note")
      (let* ((chain (scholia-core--select-chain))
             ...
             (texts (lambda ()
                      (let ((chain (car (scholia-buffer-chains))))
                        (cons (overlay-get (car chain) 'scholia-annotation)
                              (mapcar (lambda (entry)
                                        (scholia-db-annotation-text (cdr entry)))
                                      (alist-get (overlay-get (car chain)
                                                              'scholia--chain-id)
                                                 scholia--replies)))))))
        (scholia-reply-to "first" root)
        (scholia-reply-to "second" root)
        (let (asked)
          (cl-letf (((symbol-function 'completing-read)
                     (lambda (prompt candidates &rest _)
                       (setq asked prompt)
                       (car (nth 1 candidates))))
                    ((symbol-function 'scholia-ui-read-annotation)
                     (lambda (initial &rest _)
                       (should (equal initial "first"))
                       "first, revised")))
            (call-interactively #'scholia-edit-annotation))
          (should (equal asked "Edit: ")))
        (should (equal (funcall texts) '("the note" "first, revised" "second")))
        (scholia-mode -1)
        (scholia-mode 1)
        (should (equal (funcall texts) '("the note" "first, revised" "second")))))))
